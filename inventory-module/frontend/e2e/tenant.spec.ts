...
test.describe('Multi-Tenancy & Navigation', () => {
  ...
  test('should show admin warning when GLOBAL_ADMIN role is selected', async ({ page }) => {
    ...
    // Open settings and select GLOBAL_ADMIN
    ...
    // Select GLOBAL_ADMIN role
    const roleSelect = page.locator('.settings-panel select').last();
    await roleSelect.selectOption('GLOBAL_ADMIN');
